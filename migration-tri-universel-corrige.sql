-- ===================================
-- Migration pour le tri par glisser-déposer - VERSION CORRIGÉE
...
-- Initialiser les ordres pour page_content (utilise "heroTitle" avec guillemets)
WITH numbered_items AS (
...
SELECT 
    'VERIFICATION MIGRATION' as section,
    (SELECT COUNT(*) FROM accommodations WHERE sort_order > 0) as accommodations_sorted,
    (SELECT COUNT(*) FROM events WHERE sort_order > 0) as events_sorted,
    (SELECT COUNT(*) FROM articles WHERE sort_order > 0) as articles_sorted,
    (SELECT COUNT(*) FROM products WHERE sort_order > 0) as products_sorted,
    (SELECT COUNT(*) FROM page_content WHERE sort_order > 0) as page_content_sorted,
    (SELECT COUNT(*) FROM experiences WHERE sort_order > 0) as experiences_sorted;